...
def service_details(request, slug):
    ...
    context = {'service': service, 'request': request, 'other_services':other_services,}

    if request.method == 'POST':
        form = EnquiryForm(request.POST)

        if form.is_valid():
            form.instance.service = service  # Link the EnquiryForm to the specific Service
            form.save()
            response_data = {
                "status": "true",
                "title": "Successfully Submitted",
...
def update_details(request,slug):
    ...
    context = {'update': update, 'request': request, 'other_updates':other_updates,}
